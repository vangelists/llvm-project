...
enum CommandArgumentType {
  ...
  eArgTypeThreadIndex,
  eArgTypeThreadName,
  eArgTypeTracedWriteTiming,
  eArgTypeTracepointID,
  eArgTypeTracingBookmarkName,
  eArgTypeTracingBookmarkID,
  eArgTypeTypeName,
...
enum class TracingToken {
 ExpressionEvaluation, ///< Used when evaluating a user expression.
 Internal,             ///< Only for use by the tracer.
 Invalid,              ///< Only for use by the tracer.
 UserCommand           ///< Used by "thread tracing {suspend, resume}" commands.
};

/// \enum TracedWriteTiming
///
/// Indicates whether to list location of past, future or both traced
/// instructions that have modified a register, variable or heap address.
///
enum class TracedWriteTiming {
  Past,   ///< List writes made before the current tracepoint.
  Future, ///< List writes made after the current tracepoint.
  Any     ///< List writes made both before and after the current tracepoint.
};
