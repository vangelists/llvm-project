...
bool ThreadPlanTracer::TracerExplainsStop() {
  if (m_state == State::eEnabled && m_single_step) {
    return m_thread.GetStopInfo()->GetStopReason() == eStopReasonTrace;
  } else {
    return false;
  }
}
...
void ThreadPlanInstructionTracer::CaptureSnapshot() {
  ...
  Thread::ThreadPlanStack completed_plans = m_thread.GetCompletedPlanStack();
...
void ThreadPlanInstructionTracer::ClearSubsequentRecordingHistoryIfNeeded() {
  ...
  m_timeline.erase(m_timeline.begin() + m_current_tracepoint + 1,
                   m_timeline.end());
